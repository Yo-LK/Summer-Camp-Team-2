...
**当前状态：** 完整的数据处理流水线（下载 → 切分/加窗/特征提取 → 基线模型 → 自适应方法）已经完成，并生成了一张统一的结果汇总表（`data/agent_policy_table.csv`，12 个负载对 × 7 种方法），可直接用于驱动智能体的决策策略。智能体/编排层——也就是项目目标中真正"构建智能体"的部分——是下一步尚未完成的工作；目前为止的一切都只是一个手动运行的 notebook 流水线，负责生产智能体所需要的输入数据。
...
| 3 | 选择合适的故障诊断方法 | 🟢 基本完成 | 7 种方法已经在真实的逐对数据上进行了正面对比，并汇总进 `data/agent_policy_table.csv`——剩下的差距是"选择"这一步目前仍由人来读表决定，而不是由策略/智能体自动完成 |
...
- **特征提取** —— 4 种方法，在 `windows_by_load` 中的每个窗口上一次性计算完成（共 5,601 个窗口）：
  ...
  4. **故障特征频率峰值（BPFO/BPFI/BSF）** —— 在理论上的外圈/内圈/滚珠自转故障频率（CWRU 公布的 SKF 6205 驱动端轴承阶次系数：转速的 3.5848×/5.4152×/2.357×）及其 2、3 次谐波处（±5 Hz 容差范围内）读取包络谱的幅值——将密集的包络谱压缩为每个窗口 9 个具有物理意义的数值。
...
### `domain_adaptation.ipynb` —— 自适应 CNN、CORAL + 随机森林、全面对比
...
- **自适应经典机器学习 —— CORAL + 随机森林** —— 将 `source_train` 的协方差对齐到 `target_labeled` 的协方差（CORAL），在对齐后的结果上训练随机森林，共在三种特征集上运行：9 维的 `features_fault_freq`（BPFO/BPFI/BSF 峰值），以及 `features_fft`/`features_envelope`（各 2049 维，先经 PCA 降到 20 维——否则在只有 56 个目标样本的情况下，CORAL 所需的协方差矩阵会严重秩亏）。
- 一项完整性检查用同一套稀缺子集选取逻辑重新计算出基线 2 的数值，并确认与 `baseline_results.csv` 中的结果一致（差异仅来自 GPU 训练的不确定性），从而验证了 `windows_by_load.pkl`（原始窗口）与 `features_*.npz` 文件（预提取特征）之间基于索引的窗口对应关系是正确的。
- **68 个模型保存到 `models/`**：24 个自适应 CNN 检查点（2 种冻结模式 × 12 个负载对）+ 36 个 CORAL+RF 组合（3 种特征集 × 12 个负载对，以 joblib 保存 `{clf, scaler, pca}`），加上前面提到的 8 个基线检查点。
- **汇总策略表** —— 将全部 7 种方法的准确率重塑为一张宽表，每个负载对一行，每种方法一列，保存到 **`data/agent_policy_table.csv`**。这正是为下一阶段准备的交接产物（一个能够针对给定的 source→target 负载对，判断该信任哪种方法的智能体）——详见下方结果。
...
未出现任何一种方法能赢下所有负载对（例如 CNN 部分冻结在 2→3 和 3→2 上胜出；基线 2 在 1→0、2→0、3→0 上以较大优势胜出；CORAL+RF 使用故障频率特征的版本则从未在任何负载对上胜出）——这种逐对之间的差异性正是智能体策略需要据以判断的真实信号。
...
| CORAL + 随机森林（FFT 特征） | 81.5% |
| 自适应 CNN（部分冻结） | 81.0% |
| 自适应 CNN（完全冻结） | 75.7% |
| CORAL + 随机森林（包络特征） | 74.3% |
| 基线 1 —— 仅源域 | 69.7% |
| CORAL + 随机森林（故障频率特征） | 49.1% |

![按方法划分的平均准确率](assets/mean_accuracy.png)

**逐对完整结果**（全部 12 个负载对的单独数据，而不仅是上面的平均值）保存在 **`data/full_comparison_results.csv`** 以及下方两张图表中：
...
- 部分解冻 CNN 的最后一个卷积块（75.7% → 81.0%）以及将 CORAL 从紧凑的故障频率特征切换为完整的 FFT 频谱（49.1% → 81.5%），相比每种方法最初的尝试都带来了巨大而真实的提升——故障频率特征集才是 CORAL 表现不佳的瓶颈，而非 CORAL 方法本身的问题。
- 两种自适应方法平均而言都没有明显**超过**基线 2——表现最好的版本基本与之持平。就本任务而言，利用源域知识带来的收益目前还没有超过直接用同等数量的稀缺目标域数据训练所能达到的效果。
...
├── features_fault_freq.npz     # BPFO/BPFI/BSF 峰值幅度，(5601, 9)，+ 每窗口元数据
├── baseline_results.csv        # 基线 1 & 2 的准确率/宏平均 F1，每个负载对一行（共 12 行）
├── full_comparison_results.csv # 全部 7 种方法的准确率/宏平均 F1，每个负载对一行（共 12 行）
└── agent_policy_table.csv      # 宽表：每对一行 × 7 个方法列，仅含准确率 —— 智能体交接产物

models/                          # 未被 gitignore —— 共 68 个文件
...
`data/` 被 gitignore 忽略（大型二进制文件）——其中的一切都可以通过依次运行 `data_download.ipynb`、`data_splitting_preprocessing.ipynb`、`model_training.ipynb`、`domain_adaptation.ipynb` 复现。`models/` 与 `assets/` 未被忽略，因为 `assets/` 存放的是本 README 所需的图片，而 `models/` 并不像 `data/` 那样属于大型二进制数据。
...
按顺序运行：`data_download.ipynb`（填充 `data/`）、`data_splitting_preprocessing.ipynb`、`model_training.ipynb`（在 `models/` 中生成 8 个基线检查点）、`domain_adaptation.ipynb`（再向 `models/` 添加 60 个检查点/组合，重新生成 `assets/*.png`，并生成 `data/agent_policy_table.csv`）。
